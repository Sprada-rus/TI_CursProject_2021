...
void __fastcall TForm1::BitBtn1Click(TObject *Sender){
	...
	try
	{
		...
		if(!ca || cb <= 0.0 || cc <= 0.0 || cd <= 0.0) throw "Начальные концентрации компонентов должны быть больше 0";
		...
		for (i = 1; i <= count ; i++) {
			...
			if(i == 1){
				...
				Series3->AddXY(StringGrid1->Cells[1][i].ToDouble(), cb += k * (-6) *pow(ca, n) * t);
				Series4->AddXY(StringGrid1->Cells[1][i].ToDouble(), cc += k * 2 * pow(ca, n) * t);
				Series5->AddXY(StringGrid1->Cells[1][i].ToDouble(), cd += k * 2 * pow(ca, n) * t);
				if (reg_down > reg_up) {
					Series1->AddXY(StringGrid1->Cells[1][i].ToDouble(), ca -= k * 4 * pow(ca, n) * t);
				} else {
					Series1->AddXY(StringGrid1->Cells[1][i].ToDouble(), ca += k * 4 * pow(ca, n) * t);
				}
			}
